...
```text
https://raw.githubusercontent.com/STERILITZIA02/Blockad-sr-my/v1.0.0/release/v1.0.0/components/<组件名>.sgmodule
```
...
- QQ：冷启动、腾讯新闻开屏、消息、文件、语音/视频仍正常。
- 微信：公众号文章、朋友圈素材；同时检查聊天、支付、小程序和视频号。
- 京东：冷/热启动、首页、订单、物流、登录和支付。
- 淘宝/天猫：冷启动、首页、搜索、商品详情、购物车、订单和直播。
...
- Bilibili：开屏、首页、动态、播放页和直播；登录、清晰度、投币、收藏和播放。
- 通用长尾规则：出现异常的 App 应记录时间、URL/主机和功能，不要直接扩大封禁。
...
YouTube 的服务端拼接广告、抖音等第一方加密/客户端原生 feed 广告，以及 TLS
证书固定的 App，不一定能被 Shadowrocket 可靠处理。不能为了“全去除”而封禁正常
视频 CDN、全部 QUIC/UDP 或大范围共享域名。
