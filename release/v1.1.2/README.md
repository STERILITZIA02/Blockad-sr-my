...
[![一键导入 Shadowrocket](https://img.shields.io/badge/一键导入-Shadowrocket-1677FF?style=for-the-badge&logo=apple&logoColor=white)](https://sterilitzia02.github.io/Blockad-sr-my/install.html)

在 iPhone 或 iPad 的 Safari 中点击按钮，并允许网页打开 Shadowrocket。按钮经由
本仓库 GitHub Pages 的固定跳转页安装稳定入口；跳转页不接收任意外部 URL。若系统
未自动唤起，页面会显示“再次打开 Shadowrocket”和原始订阅链接。
...
```text
https://raw.githubusercontent.com/STERILITZIA02/Blockad-sr-my/v1.1.2/release/v1.1.2/BlockAd.Unified.sgmodule
```
...
- 固定版本发行目录：`release/v1.1.2/`
- 166 个 App/功能独立组件：`release/v1.1.2/components/`
- 默认关闭的便利模块：`release/v1.1.2/extras/`
- 可复现构建输入与第三方原始快照：`release/v1.1.2/source/`
- 发行目录 SHA-256：`release/v1.1.2/SHA256SUMS.txt`
- GitHub Release 包：`BlockAd-SR-v1.1.2.zip`
...
- 所有发行组件的 MITM 都使用精确主机；构建会拒绝任何通配 MITM。
...
`release:build` 会从固定上游快照重建 v1.1.2、生成稳定入口、打包 ZIP 并复核全部
