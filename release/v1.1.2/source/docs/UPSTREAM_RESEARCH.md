...
| 抖音 / 字节系 | `ByteDance.sgmodule` + 2026 设备日志样本 | 撤销会让主 feed/评论进入解密范围的 `amemv/snssdk` 通配 MITM；只保留穿山甲/Gromore 精确广告载荷和精确素材主机 |
...
- `LOWERTOP/Shadowrocket` 当前手册确认，模块 `[MITM] hostname = %APPEND% ...`
  会把列出的 HTTPS 主机加入解密范围。因此，即使 URL Rewrite 只写 `/ad/`，
  `*.amemv.com` 与 `*.snssdk.com` 仍会让同主机上的 feed、评论和发布连接进入
  MITM，不能视为“只影响广告路径”。
- `Y123456-hzy/gy-adblock-mitm-lite.sgmodule` 修订
  `69c5fc3590814d39fd9518bff204315371f7b39c`（2026-06）记录了设备日志结论：
  对 GDT、穿山甲、友盟、Bugly 等 SDK 握手/配置域做整域拒绝会触发重试并可能
  卡住 App；其当前方案使用 DIRECT 保护基础连接，再用精确 URL Rewrite 清空广告
  载荷。本项目没有复制无许可证 Gist 代码，只采用了这一可验证的故障模式、当前
  主机拓扑与端点事实，并通过本地独立配置实现。
- `blackmatrix7/ios_rule_script` 的 Shadowrocket 抖音域列表仍把 `amemv.com` 与
  `snssdk.com` 归为抖音业务域，这与本次“不可通配解密”的判断相符。

选择结果：v1.1.2 保留 `get_ads/settings`、Gromore `config/exchange`、
GDT `gdt_mview/view/click`、穿山甲落地页和两个专用素材主机；撤下第一方
`amemv/snssdk` 广告路径 Rewrite，因为为它们启用 MITM 的功能风险高于可证明收益。
