...
在 iPhone 或 iPad 的 Safari 中打开 GitHub 仓库，点击 README 顶部的
“一键导入 Shadowrocket”按钮；也可直接打开：

```text
https://sterilitzia02.github.io/Blockad-sr-my/install.html
```

页面会请求打开 Shadowrocket，并安装下方稳定模块。若系统没有自动唤起，请点击
页面中的“再次打开 Shadowrocket”。不要在非 iOS 浏览器中把页面跳转失败误判为
订阅失效。
...
```text
https://raw.githubusercontent.com/STERILITZIA02/Blockad-sr-my/v1.1.2/release/v1.1.2/components/<组件名>.sgmodule
```
...
`release/v1.1.2/manifest.json`；原软件名到当前组件的对应关系见
`docs/APP_PARITY.md`。只启用独立组件时，不要再启用统一模块或重复组件。

富途 `futu` 组件因涉及金融交易 API，不默认并入统一模块。只有确认当前 App
版本可正常使用 HTTPS 解密、并愿意完成行情/下单/撤单/登录真机验收时才应单独启用。
...
如果从 v1.1.1 升级是为修复抖音刷新/评论问题，请在更新模块后完全结束抖音进程
再重开。若问题仍在，先确认模块详情版本为 `1.1.2`，再停用统一模块对照测试；
不要同时保留旧 `douyin` 独立组件。v1.1.2 不再对 `amemv.com` 或 `snssdk.com`
执行 MITM，日志中若仍显示这些主机被解密，应检查其他已启用模块。
