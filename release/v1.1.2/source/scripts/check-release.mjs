...
async function main() {
  ...
  const releasePaths = actualFiles.map((file) => posix(path.relative(RELEASE, file)));
  assert.equal(
    releasePaths.some(
      (relative) =>
        relative.startsWith("source/现有插件/") ||
        /^source\/.+\.(?:sg)?module$/i.test(relative),
    ),
    false,
    "发行包 source 不得携带可误导导入的旧模块",
  );
  for (const required of [
    "audit/legacy-inventory.json",
    "audit/app-parity.json",
    "docs/APP_PARITY.md",
    ...
    "docs/install.html",
    "docs/.nojekyll",
    "source/config/apps.mjs",
    "source/config/legacy-apps.mjs",
    "source/scripts/build.mjs",
    "source/test/router.test.mjs",
    "source/docs/index.html",
    "source/docs/install.html",
    "source/docs/.nojekyll",
  ]) {
    assert.equal(releasePaths.includes(required), true, `发行包缺少 ${required}`);
  }
  const modules = actualFiles.filter((file) => {
    if (!file.endsWith(".sgmodule")) return false;
    const relative = posix(path.relative(RELEASE, file));
    return (
      relative === "BlockAd.Unified.sgmodule" ||
      relative.startsWith("components/") ||
      relative.startsWith("extras/")
    );
  });
  assert.equal(
    modules.length,
    apps.length + optionalExtras.length + 1,
    "固定版本模块数量不一致",
  );
  const runtimeFiles = actualFiles.filter((file) => {
    const relative = posix(path.relative(RELEASE, file));
    return (
      (relative === "BlockAd.Unified.sgmodule" ||
        ...
        relative.startsWith("rules/")) &&
      /\.(?:sgmodule|js|list|json)$/i.test(relative)
    );
  });
  for (const file of runtimeFiles) {
    ...
    assert.equal(
      ...
      false,
      `${posix(path.relative(ROOT, file))}: 固定版运行文件不得包含 Bilibili 内容`,
    );
  }
