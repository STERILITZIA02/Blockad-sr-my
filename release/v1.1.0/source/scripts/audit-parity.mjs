  ...
  return new Map(
    [...apps, ...optionalExtras].map((app) => [
      app.id,
      {
        ...
        legacyGroups: app.legacyGroups ?? [],
        auditNote: app.auditNote,
        auditDisposition: app.auditDisposition,
      },
    ]),
  );
}

function buildCoverageIndex(currentApps) {
  const coverage = new Map(
    Object.entries(legacyCoverage).map(([name, value]) => [
      name,
      { ...value, components: [...value.components] },
    ]),
  );

  for (const [id, app] of currentApps) {
    for (const groupName of app.legacyGroups) {
      const existing = coverage.get(groupName);
      if (existing) {
        if (!existing.components.includes(id)) {
          existing.components.push(id);
        }
        continue;
      }
      coverage.set(groupName, {
        components: [id],
        disposition: app.auditDisposition ?? "migrated",
        note:
          app.auditNote ??
          "原规则已改写为本仓库内的精确广告入口，并由独立组件提供开关。",
      });
    }
  }
  return coverage;
...
function dispositionForGroup(group, coverage) {
  const override = legacyDecisionOverrides[group.name];
  if (override) return override;
  const mapped = coverage.get(group.name);
...
function remoteScriptDisposition(url, appIds) {
  ...
  if (/bili_auto_regions|auto_regions/.test(token)) {
    return {
      disposition: "not-executed",
      components: [],
      note: "地区解锁会改写播放区域并执行外部查询，不属于去广告稳定路径。",
    };
  }
  if (/bili/.test(token)) {
    return {
      disposition: "not-executed",
      components: [],
      note: "用户已有专用 Bilibili 规则；本仓库不执行或替代这类脚本。",
    };
  }
  const replacements = [
    { pattern: /wechatad|weixin|wechat/, apps: ["wechat"] },
    { pattern: /weibo/, apps: ["weibo"] },
    { pattern: /qq-news/, apps: ["qq"] },
    { pattern: /xianyu/, apps: ["xianyu"] },
    { pattern: /amap/, apps: ["amap"] },
    { pattern: /zheye|zhihu/, apps: ["zhihu"] },
    { pattern: /jd_json|\bjd\b|jingdong/, apps: ["jd"] },
    { pattern: /youtube/, apps: ["youtube"] },
    { pattern: /tieba/, apps: ["baidu-tieba"] },
    { pattern: /redbook|xiaohongshu/, apps: ["xiaohongshu"] },
    { pattern: /smzdm/, apps: ["smzdm"] },
    { pattern: /ximalaya|xmly/, apps: ["ximalaya"] },
    { pattern: /coolapk/, apps: ["coolapk"] },
    { pattern: /cainiao/, apps: ["cainiao"] },
    { pattern: /quark/, apps: ["quark"] },
    { pattern: /didi/, apps: ["didi"] },
    { pattern: /netease\.adblock|netease/, apps: ["netease-cloudmusic"] },
    { pattern: /kuwo/, apps: ["kuwo"] },
    { pattern: /boohee|bohe/, apps: ["boohee"] },
  ];
...
async function main() {
  ...
  const coverage = buildCoverageIndex(currentApps);

  const groupRows = groups.map((group) => {
    const decision = dispositionForGroup(group, coverage);
